...
class Command(ABC):
    ...
    @classmethod
    def create_from_params(cls, params: dict, keyboard = None):
        return cls()
...
class VolumeUpCommand(Command):
    ...
    def execute(self) -> None:
        print(f"volume increased {self.value}%")

    def cancel(self) -> None:
        print(f"volume decreased {self.value}%")
    
    def get_params(self) -> dict:
        return {"value": self.value}

    @classmethod
    def create_from_params(cls, params: dict):
        return cls(value = params.get("value"))
...
class DotaPlayerCommand(Command):
    def execute(self) -> None:
        print("Dota started")
...
class SaveKeyboardToFile():
    ...
    def save_to_xlsx(self) -> None:
        ...
        row = 7
        for key, command in self.keyboard.commands.items():
            serialized = {
                "class": command.__class__.__name__,
                "params": command.get_params()
            }
            note[f'A{row}'] = key
            note[f'B{row}'] = str(serialized)
            row += 1 
    ...
    def load_from_xlsx(self) -> None:
        try:
            ...
            print(f"File {self.file_dir} not found")
            return
        
        note = wb.active
        self.keyboard.output.text = note['B2'].value or ""

        commands_stack = note['B3'].value or "[]"
        self.keyboard.output.commands_stack = ast.literal_eval(commands_stack)

        index_str = note['B4'].value or "-1"
        ...
        while note[f'A{row}'].value:
            ...
            try:
                ...
                if class_name == "KeyCommand":
                    command = cls(self.keyboard)
                else:
                    if hasattr(cls, 'create_from_params'):
                        command = cls.create_from_params(params)
                    else:
                        command = cls()

                commands[key] = command
            except Exception as e:
                print(f"Error with command: '{key}': {e}")
...
commands = {
    ...
    'l': KeyCommand(k),
    'o': KeyCommand(k),
    'ctrl+V++': VolumeUpCommand(50),
    'ctrl+D+-': VolumeDownCommand(),
    'ctrl+D': DotaPlayerCommand(),
}
